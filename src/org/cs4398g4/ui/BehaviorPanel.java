...
public class BehaviorPanel extends JPanel {
	 ...
	 * 
	 */
	private static final long serialVersionUID = -2838311740947277647L;
	private JTextField sensorDuration;
	private LocalInterface view;
	private JComboBox<Sensor> sensorComboBox;
	private JComboBox<PinState> sensorStateComboBox;
	private JList<Condition> conditionList;
	private DefaultListModel<Condition> conditionListModel;
	private JButton addConditionBtn;
	private JButton removeConditionBtn;
	private JTextField actuatorDurationTxt;
	private JComboBox<Actuator> actuatorComboBox;
	private JComboBox<PinState> actuatorStateComboBox;
	private JButton addActionBtn;
	private JButton removeActionBtn;
	private DefaultComboBoxModel<Sensor> sensorListModel;
	private DefaultComboBoxModel<PinState> sensorStateListModel;
	private DefaultComboBoxModel<PinState> actionStateListModel;
	private DefaultComboBoxModel<Actuator> actuatorListModel;
	private DefaultListModel<Action> actionListModel;
	private JList<Action> actionList;
	private JTextField txtNewBehavior;
	private JButton btnSave;
	private JList<Behavior> behaviorList;
	private DefaultListModel<Behavior> behaviorListModel;
	...
	public BehaviorPanel(LocalInterface view) {
		...
		gbl_behaviorListStatusPanel.columnWidths = new int[] { 218, 0, 170, 10,
				0 };
		gbl_behaviorListStatusPanel.rowHeights = new int[] { 0, 74, 0, 62, 0,
				0, 0, 0 };
		gbl_behaviorListStatusPanel.columnWeights = new double[] { 1.0, 0.0,
				1.0, 0.0, Double.MIN_VALUE };
		gbl_behaviorListStatusPanel.rowWeights = new double[] { 0.0, 1.0, 0.0,
		...
		behaviorListStatusPanel.add(behaviorSensorPanel,
				gbc_behaviorSensorPanel);
		behaviorSensorPanel
				.setLayout(new FormLayout(new ColumnSpec[] {
						FormFactory.RELATED_GAP_COLSPEC,
						ColumnSpec.decode("right:30dlu"),
						FormFactory.LABEL_COMPONENT_GAP_COLSPEC,
						ColumnSpec.decode("89px:grow"), }, new RowSpec[] {
						FormFactory.RELATED_GAP_ROWSPEC,
						FormFactory.DEFAULT_ROWSPEC,
						FormFactory.LINE_GAP_ROWSPEC, RowSpec.decode("22px"),
						FormFactory.RELATED_GAP_ROWSPEC,
						FormFactory.DEFAULT_ROWSPEC,
						FormFactory.RELATED_GAP_ROWSPEC,
						FormFactory.DEFAULT_ROWSPEC, }));
		...
		sensorStateComboBox = new JComboBox<PinState>();
		...
		gbl_conditionBtnPanel.columnWeights = new double[] { 1.0,
				Double.MIN_VALUE };
		gbl_conditionBtnPanel.rowWeights = new double[] { 0.0, 0.0,
				Double.MIN_VALUE };
		...
		conditionList = new JList<Condition>();
		...
		behaviorListStatusPanel.add(behaviorActuatorPanel,
				gbc_behaviorActuatorPanel);
		behaviorActuatorPanel
				.setLayout(new FormLayout(new ColumnSpec[] {
						FormFactory.RELATED_GAP_COLSPEC,
						ColumnSpec.decode("right:30dlu"),
						FormFactory.RELATED_GAP_COLSPEC,
		...
		actuatorComboBox = new JComboBox<Actuator>();
		...
		// ComponentState[] stateOptions = {new ComponentState("High", 1), new
		// ComponentState("Low", 0)};
		actuatorStateComboBox = new JComboBox<PinState>();
		...
		gbl_actionBtnPanel.rowWeights = new double[] { 0.0, 0.0,
				Double.MIN_VALUE };
		...
		behaviorListStatusPanel.add(behaviorNameEntryPanel,
				gbc_behaviorNameEntryPanel);
		behaviorNameEntryPanel
				.setLayout(new FormLayout(new ColumnSpec[] {
						FormFactory.RELATED_GAP_COLSPEC,
		...
		behaviorListStatusPanel.add(behaviorStatusSaveBtnPanel,
				gbc_behaviorStatusSaveBtnPanel);
		behaviorStatusSaveBtnPanel.setLayout(new FlowLayout(FlowLayout.CENTER,
				5, 5));
		...
		gbl_behaviorListPanel.columnWeights = new double[] { 1.0,
				Double.MIN_VALUE };
		gbl_behaviorListPanel.rowWeights = new double[] { 0.0, 1.0, 0.0,
				Double.MIN_VALUE };
		...
		behaviorList = new JList<Behavior>();
	...
	void Initialize() {
		...
		sensorStateListModel = new DefaultComboBoxModel<PinState>(stateOptions);
		actionStateListModel = new DefaultComboBoxModel<PinState>(stateOptions);
	...
	private void SetupConditionUI() {
		List<Sensor> sensorOptions = view.getController().getComponents(
				Sensor.class);
		sensorListModel = new DefaultComboBoxModel(sensorOptions.toArray());
		// sensorListModel = new
		// DefaultComboBoxModel<Sensor>(sensorOptions.toArray(Sensor[]));
		sensorComboBox.setModel(sensorListModel);
		sensorStateComboBox.setModel(sensorStateListModel);
		conditionListModel = new DefaultListModel<Condition>();
		...
		addConditionBtn.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				// stateComboBox
				// sensorDuration
				Condition condition = new Condition((Sensor) sensorComboBox
						.getSelectedItem(), (PinState) sensorStateComboBox
						.getSelectedItem(), Integer.valueOf(sensorDuration
						.getText()));
	...
	public void fillOptions() {
		...
		List<Actuator> actuatorOptions = view.getController().getComponents(
				Actuator.class);
		...
		if (behaviorListModel.getSize() > 0)
	...
	private void SetupActionUI() {
		...
		addActionBtn.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				// stateComboBox
				// sensorDuration
				Action action = new Action((Actuator) actuatorComboBox
						.getSelectedItem(), (PinState) actuatorStateComboBox
						.getSelectedItem(), Integer.valueOf(actuatorDurationTxt
						.getText()));
	...
	void SetupBehaviorUI() {
		behaviorListModel = new DefaultListModel<Behavior>();
		...
		btnSave.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				if ((actionListModel.getSize() == 0)
						|| (conditionListModel.getSize() == 0)) {
					view.displayErrorPrompt(
							"Missing Behavior Components",
							"All new behaviors require at least one condition and at least one action to be created");
					return;
				}
				for (int i = 0; i < actionListModel.getSize(); i++) {
					actions.add(actionListModel.getElementAt(i));
				}
				List<Condition> conditions = new ArrayList<Condition>();
				for (int i = 0; i < conditionListModel.getSize(); i++) {
					conditions.add(conditionListModel.getElementAt(i));
				}
				Behavior behavior = new Behavior(txtNewBehavior.getText(),
						conditions, actions, view.getBaseStation());
		...
		btnBehaviorRemove.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				Behavior removedBehavior = behaviorListModel
						.getElementAt(index);
	...
	private void removeCondition() {
		...
		Sensor removedSensor = conditionListModel.getElementAt(index)
				.getSensor();
		...
		// System.out.println("Removing condition, conditionListModel.getSize()=>"
		// + conditionListModel.getSize());
	...
	private void removeAction() {
		...
		if (index == -1)
			index = 0;
		Actuator removedActuator = actionListModel.getElementAt(index)
				.getActuator();
		...
		// System.out.println("Removing action, actionListModel.getSize()=>" +
		// size);
